...
public class Keychain {
    ...
    public func isExpired(key: String, expirationInterval expiration: NSTimeInterval) -> Bool {
        let failable = isExpiredOrError(key, expirationInterval: expiration)
        if var result = failable.value {
            return result
        }
        return false
    }
    
    public func isExpiredOrError(key: String, expirationInterval expiration: NSTimeInterval) -> FailableOf<Bool> {
        var query = options.query()
        ...
        func interval(date: NSDate) -> NSTimeInterval
        {
        ...
        switch status {
        case errSecSuccess:
            if let date = result?[kSecAttrModificationDate as! String] as? NSDate {
                return FailableOf(interval(date) >= expiration)
            }
            if let date = result?[kSecAttrCreationDate as! String] as? NSDate {
                return FailableOf(interval(date) >= expiration)
            }
            return FailableOf(securityError(status: Status.UnexpectedError.rawValue))
        case errSecItemNotFound:
